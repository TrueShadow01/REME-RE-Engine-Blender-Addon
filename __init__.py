...
from .modules.gen_functions import (
    formatByteSize,
    getFolderSize,
    openFolder,
    raiseWarning,
...
from .modules.mdf.blender_re_mdf import exportMDFFile, importMDFFile
...
from .modules.mesh.blender_re_mesh import exportREMeshFile, importREMeshFile
...
# sfur
from .modules.sfur.blender_re_sfur import exportSFurFile, importSFurFile
...
def setMeshImportDefaults(self):
    self.clearScene = bpy.context.preferences.addons[
        __name__
    ].preferences.default_clearScene
    self.loadMaterials = bpy.context.preferences.addons[
        __name__
    ].preferences.default_loadMaterials
    self.loadMDFData = bpy.context.preferences.addons[
        __name__
    ].preferences.default_loadMDFData
    self.loadShellFur = bpy.context.preferences.addons[
        __name__
    ].preferences.default_loadShellFur
    self.loadUnusedTextures = bpy.context.preferences.addons[
        __name__
    ].preferences.default_loadUnusedTextures
    self.loadUnusedProps = bpy.context.preferences.addons[
        __name__
    ].preferences.default_loadUnusedProps
    self.useBackfaceCulling = bpy.context.preferences.addons[
        __name__
    ].preferences.default_useBackfaceCulling
    self.reloadCachedTextures = bpy.context.preferences.addons[
        __name__
    ].preferences.default_reloadCachedTextures
    self.createCollections = bpy.context.preferences.addons[
        __name__
    ].preferences.default_createCollections
    self.importArmatureOnly = bpy.context.preferences.addons[
        __name__
    ].preferences.default_importArmatureOnly
    self.importAllLODs = bpy.context.preferences.addons[
        __name__
    ].preferences.default_importAllLODs
    self.rotate90 = bpy.context.preferences.addons[
        __name__
    ].preferences.default_rotate90
    self.importBlendShapes = bpy.context.preferences.addons[
        __name__
    ].preferences.default_importBlendShapes
    self.importShadowMeshes = bpy.context.preferences.addons[
        __name__
    ].preferences.default_importShadowMeshes
    self.mergeGroups = bpy.context.preferences.addons[
        __name__
    ].preferences.default_mergeGroups
    self.importOcclusionMeshes = bpy.context.preferences.addons[
        __name__
    ].preferences.default_importOcclusionMeshes
    self.importBoundingBoxes = bpy.context.preferences.addons[
        __name__
    ].preferences.default_importBoundingBoxes
    # print("RE Mesh Editor: Loaded Default Import Settings")


def setMeshExportDefaults(self):
    self.selectedOnly = bpy.context.preferences.addons[
        __name__
    ].preferences.default_selectedOnly
    ...
    self.autoSolveRepeatedUVs = bpy.context.preferences.addons[
        __name__
    ].preferences.default_autoSolveRepeatedUVs
    self.preserveSharpEdges = bpy.context.preferences.addons[
        __name__
    ].preferences.default_preserveSharpEdges
    self.useBlenderMaterialName = bpy.context.preferences.addons[
        __name__
    ].preferences.default_useBlenderMaterialName
    self.preserveBoneMatrices = bpy.context.preferences.addons[
        __name__
    ].preferences.default_preserveBoneMatrices
    self.exportBoundingBoxes = bpy.context.preferences.addons[
        __name__
    ].preferences.default_exportBoundingBoxes
...
def setModDirectoryFromFilePath(filePath):
    if (
        "re_chunk_000" not in filePath
        and "re_dlc_stm" not in filePath
        and "natives" in filePath
    ):
        try:
            bpy.context.scene.re_mdf_toolpanel.modDirectory = splitNativesPath(
                filePath
            )[0]
            print(
                f"Set mod directory to {bpy.context.scene.re_mdf_toolpanel.modDirectory}"
            )
        except Exception as e:
            print("ERROR: Failed to set mod directory - {str(e)}")
            print(
                "Exported file path probably does not follow the chunk naming schema."
            )
...
class WM_OT_ClearTextureCacheFolder(Operator):
    ...
    def draw(self, context):
        ...
        layout.label(
            text=f"{bpy.path.abspath(bpy.context.preferences.addons[__name__].preferences.textureCachePath)}"
        )
    ...
    def execute(self, context):
        ...
        for root, dirs, files in os.walk(
            bpy.path.abspath(
                bpy.context.preferences.addons[__name__].preferences.textureCachePath
            )
        ):
            for file in files:
...
class AddItemOperator(bpy.types.Operator):
    ...
    bl_description = (
        "Add path to the extracted chunk's natives\STM\ folder.\n"
        + r"Example: I:\RE4_EXTRACT\re_chunk_000\natives\STM"
    )
    bl_label = "Add Path"
...
class ReorderItemOperator(bpy.types.Operator):
    ...
    def move_index(self):
        ...
        list_length = len(
            bpy.context.preferences.addons[__name__].preferences.chunkPathList_items
        )
        new_index = index + (-1 if self.direction == "UP" else 1)
        bpy.context.preferences.addons[__name__].preferences.chunkPathList_index = max(
...
class REMeshPreferences(AddonPreferences):
    ...
    textureCachePath: StringProperty(
        ...
        default=os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "TextureCache"
        ),
    )
    useDDS: BoolProperty(
    ...
    showImportOptions: BoolProperty(name="Show Default Import Settings", default=False)
    showExportOptions: BoolProperty(name="Show Default Export Settings", default=False)
    ...
    def draw(self, context):
        ...
        col2 = split.column()
        col3 = split.column()
        op = col2.operator("wm.url_open", text="Donate on Ko-fi", icon="FUND")
        ...
        if len(self.textureCachePath) > 110:
            ...
            box.label(
                text="File paths may exceed the max length of 255 characters and fail to convert."
            )
            box.label(
                text="Consider changing this to a shorter path such as C:\REMeshTextureCache."
            )
        else:
            row.prop(self, "textureCachePath")
        row = box.row()
        ...
        row = layout.row(align=True)
        row.operator(
            "re_mesh_cm.chunk_path_list_reorder_item", text="Move Up"
        ).direction = "UP"
        row.operator(
            "re_mesh_cm.chunk_path_list_reorder_item", text="Move Down"
        ).direction = "DOWN"
...
class ImportREMesh(Operator, ImportHelper):
    ...
    showAdvancedOptions: BoolProperty(name="Show Advanced Options", default=False)
    showMaterialOptions: BoolProperty(name="Show Material Options", default=True)
    ...
    def execute(self, context):
        try:
            ...
            raiseWarning(
                "Could not create texture cache directory at "
                + bpy.context.preferences.addons[__name__].preferences.textureCachePath
            )
        ...
        options = {
            ...
            "createCollections": self.createCollections,
            "loadMaterials": self.loadMaterials,
            "loadMDFData": self.loadMDFData,
            "loadShellFur": self.loadShellFur,
            ...
            "mergeArmature": self.mergeArmature,
            "importArmatureOnly": self.importArmatureOnly,
            "mergeGroups": self.mergeGroups,
            "importShadowMeshes": self.importShadowMeshes,
        ...
        if multiFileImport and self.mergeImportedArmatures:
            ...
            while (
                mergeArmatureName in bpy.data.armatures
            ):  # Get the name of armature that will be imported, this accounts for if there's already an armature with that name imported
                mergeArmatureName = (
                    baseMergeArmatureName + "." + str(mergeArmatureNameIndex).zfill(3)
                )
                mergeArmatureNameIndex += 1
        ...
        for index, file in enumerate(self.files):
            ...
            if os.path.isfile(filepath):
                ...
                options["clearScene"] = (
                    False  # Disable clear scene after first mesh is imported
                )
                if not success:
                ...
                hasImportErrors = True
                raiseWarning(
                    f"Path does not exist, cannot import file. If you are importing multiple files at once, they must all be in the same directory.\nInvalid Path:{filepath}"
                )
        ...
        if not hasImportErrors:
            ...
            self.report(
                {"INFO"}, "Failed to import RE Mesh. Check the console for errors."
            )
            return {"CANCELLED"}
    ...
    def invoke(self, context, event):
        ...
        if self.directory:
            if bpy.context.preferences.addons[
                __name__
            ].preferences.dragDropImportOptions:
...
def update_targetMeshCollection(self, context):
    ...
    if browserSpace != None:
        ...
        if ".mesh" in self.targetCollection:
            browserSpace.params.filename = (
                self.targetCollection.split(".mesh")[0] + ".mesh" + self.filename_ext
            )
...
class ExportREMesh(Operator, ExportHelper):
    ...
    filename_ext: EnumProperty(
        ...
        items=[
            ...
            (
                ".2109108288",
                "Resident Evil 2 / 3 Ray Tracing",
                "Resident Evil 2/3 Ray Tracing Version",
            ),
            (
                ".220128762",
    ...
    def invoke(self, context, event):
        ...
        if (
            context.scene.get("REMeshLastImportedMeshVersion", 0)
            in meshFileVersionToGameNameDict
        ):
        ...
        if (
            context.scene.get("REMeshLastExportedMeshVersion", 0)
            in meshFileVersionToGameNameDict
        ):
        ...
        if self.targetCollection == "":
            ...
            if exportCollection in bpy.data.collections:
                ...
                if (
                    ".mesh" in exportCollection
                ):  # Remove blender suffix after .mesh if it exists
                    if (
                        context.scene.get("REMeshLastExportedMeshVersion")
                        in meshFileVersionToGameNameDict
                    ):
                        self.filename_ext = "." + str(
                            context.scene.get(
                                "REMeshLastExportedMeshVersion", 1808282334
                            )
                        )
                    self.filepath = (
                        exportCollection.split(".mesh")[0] + ".mesh" + self.filename_ext
                    )
            else:
                ...
                if (
                    ".mesh" in prevCollection
                ):  # Remove blender suffix after .mesh if it exists
                    if (
                        context.scene.get("REMeshLastImportedMeshVersion")
                        in meshFileVersionToGameNameDict
                    ):
                        self.filename_ext = "." + str(
                            context.scene.get(
                                "REMeshLastImportedMeshVersion", 1808282334
                            )
                        )
                    self.filepath = (
                        prevCollection.split(".mesh")[0] + ".mesh" + self.filename_ext
                    )
    ...
    def draw(self, context):
        ...
        if self.targetCollection in bpy.data.collections:
            ...
            if not collection.get(
                "~TYPE"
            ) == "RE_MESH_COLLECTION" and not collection.name.endswith(".mesh"):
                row = layout.row()
                row.alert = True
    ...
    def execute(self, context):
        ...
        if success:
            ...
            if self.targetCollection in bpy.data.collections:
                bpy.data.collections[self.targetCollection]["BatchExport_path"] = (
                    self.filepath
                )
                bpy.data.collections[self.targetCollection][
                    "BatchExport_exportAllLODs"
                ] = self.exportAllLODs
                bpy.data.collections[self.targetCollection][
                    "BatchExport_preserveSharpEdges"
                ] = self.preserveSharpEdges
                ...
                ] = self.preserveBoneMatrices
                bpy.data.collections[self.targetCollection][
                    "BatchExport_exportBoundingBoxes"
                ] = self.exportBoundingBoxes
        else:
            self.report(
                {"INFO"},
                "RE Mesh export failed. See Window > Toggle System Console for info on how to fix it.",
            )
...
class ImportREMDF(bpy.types.Operator, ImportHelper):
    ...
    def execute(self, context):
        ...
        if not hasImportErrors:
            ...
            self.report(
                {"INFO"}, "Failed to import RE MDF. Check the console for errors."
            )
            return {"CANCELLED"}
...
def update_targetMDFCollection(self, context):
    ...
    if browserSpace != None:
        ...
        if ".mdf2" in self.targetCollection:
            browserSpace.params.filename = (
                self.targetCollection.split(".mdf2")[0] + ".mdf2" + self.filename_ext
            )
...
class ExportREMDF(bpy.types.Operator, ExportHelper):
    ...
    filename_ext: EnumProperty(
        ...
        items=[
            ...
            (
                ".40",
                "Dragon's Dogma 2 / Kunitsu-Gami / Dead Rising",
                "Dragon's Dogma 2, Kunitsu-Gami, Dead Rising",
            ),
        ...
        if bpy.data.collections.get(self.targetCollection, None) == None:
            if bpy.context.scene.re_mdf_toolpanel.mdfCollection:
                self.targetCollection = (
                    bpy.context.scene.re_mdf_toolpanel.mdfCollection.name
                )
                if self.targetCollection.endswith(".mdf2"):
                    self.filepath = self.targetCollection + self.filename_ext
            self.filename_ext = "." + str(
                gameNameMDFVersionDict[bpy.context.scene.re_mdf_toolpanel.activeGame]
            )
        context.window_manager.fileselect_add(self)
        return {"RUNNING_MODAL"}
    ...
    def draw(self, context):
        ...
        if self.targetCollection in bpy.data.collections:
            ...
            if not collection.get(
                "~TYPE"
            ) == "RE_MDF_COLLECTION" and not collection.name.endswith(".mdf2"):
                row = layout.row()
                row.alert = True
    ...
    def execute(self, context):
        ...
        if success:
            ...
            self.report(
                {"INFO"},
                "Failed to export RE MDF. Check Window > Toggle System Console for details.",
            )
            return {"CANCELLED"}
...
def update_targetFBXSkelArmature(self, context):
    ...
    if browserSpace != None:
        ...
        if ".fbxskel" in self.targetArmature:
            browserSpace.params.filename = (
                self.targetArmature.split(".fbxskel")[0]
                + ".fbxskel"
                + self.filename_ext
            )
...
class ExportREFBXSkel(bpy.types.Operator, ExportHelper):
        ...
        if self.targetArmature == "":
            if context.active_object != None:
                ...
                for obj in bpy.context.scene.objects:
                    if obj.type == "ARMATURE" and (
                        ".fbxskel" in obj.name
                        or ".skeleton" in obj.name
                        or ".refskel" in obj.name
                    ):
            ...
            if self.targetArmature != "":
                if ".fbxskel" in self.targetArmature:
                    ...
                    self.filepath = (
                        self.targetArmature.split(".skeleton")[0]
                        + ".skeleton"
                        + self.filename_ext
                    )
                elif ".refskel" in self.targetArmature:
                    self.filepath = (
                        self.targetArmature.split(".refskel")[0]
                        + ".refskel"
                        + self.filename_ext
                    )
        context.window_manager.fileselect_add(self)
    ...
    def draw(self, context):
        ...
        layout.prop_search(
            self, "targetArmature", bpy.data, "objects", icon="ARMATURE_DATA"
        )
    ...
    def execute(self, context):
        ...
        if success:
            ...
            self.report(
                {"INFO"},
                "Failed to export RE FBXSkel. Check Window > Toggle System Console for details.",
            )
            return {"CANCELLED"}
...
class ImportRESFur(bpy.types.Operator, ImportHelper):
    ...
    def execute(self, context):
        ...
        if not hasImportErrors:
            ...
            self.report(
                {"INFO"}, "Failed to import RE SFur. Check the console for errors."
            )
            return {"CANCELLED"}
...
def update_targetSFurCollection(self, context):
    ...
    if browserSpace != None:
        ...
        if ".sfur" in self.targetCollection:
            browserSpace.params.filename = (
                self.targetCollection.split(".sfur")[0] + ".sfur" + self.filename_ext
            )
...
class ExportRESFur(bpy.types.Operator, ExportHelper):
    ...
    def invoke(self, context, event):
        if context.scene.get("REMeshLastImportedSFurVersion", 0) != 0:
            self.filename_ext = "." + str(
                context.scene.get("REMeshLastImportedSFurVersion")
            )

        if bpy.data.collections.get(self.targetCollection, None) == None:
            if bpy.context.scene.re_mdf_toolpanel.sFurCollection:
                self.targetCollection = (
                    bpy.context.scene.re_mdf_toolpanel.sFurCollection.name
                )
    ...
    def draw(self, context):
        ...
        if self.targetCollection in bpy.data.collections:
            ...
            if not collection.get(
                "~TYPE"
            ) == "RE_SFUR_COLLECTION" and not collection.name.endswith(".sfur"):
                row = layout.row()
                row.alert = True
    ...
    def execute(self, context):
        ...
        if success:
            ...
            self.report(
                {"INFO"},
                "Failed to export RE SFur. Check Window > Toggle System Console for details.",
            )
            return {"CANCELLED"}
...
class GU_PT_collection_custom_properties(
    bpy.types.Panel, PropertyPanel
):  # For adding custom properties to collections, fixed on 3.3 and up so not needed there
...
class IMPORT_MT_re_mesh_editor_community(bpy.types.Menu):
    ...
    def draw(self, context):
        ...
        layout.operator(
            ImportREMesh.bl_idname, text="RE Mesh (.mesh.x) (Model)", icon="MESH_DATA"
        )
        layout.operator(
            ImportREMDF.bl_idname, text="RE MDF (.mdf2.x) (Materials)", icon="MATERIAL"
        )
        layout.operator(
            ImportREFBXSkel.bl_idname,
            text="RE FBXSkel (.fbxskel.x) (Skeleton)",
            icon="ARMATURE_DATA",
        )
        layout.operator(
            ImportRESFur.bl_idname,
            text="RE SFur (.sfur.x) (Shell Fur)",
            icon="CURVES_DATA",
        )
...
class EXPORT_MT_re_mesh_editor(bpy.types.Menu):
    ...
    def draw(self, context):
        ...
        layout.operator(
            ExportREMesh.bl_idname, text="RE Mesh (.mesh.x) (Model)", icon="MESH_DATA"
        )
        layout.operator(
            ExportREMDF.bl_idname, text="RE MDF (.mdf2.x) (Materials)", icon="MATERIAL"
        )
        layout.operator(
            ExportREFBXSkel.bl_idname,
            text="RE FBXSkel (.fbxskel.x) (Skeleton)",
            icon="ARMATURE_DATA",
        )
        layout.operator(
            ExportRESFur.bl_idname,
            text="RE SFur (.sfur.x) (Shell Fur)",
            icon="CURVES_DATA",
        )
