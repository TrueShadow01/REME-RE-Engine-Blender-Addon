...
class ContentFlagsA(ctypes.Union):
    _anonymous_ = ("flags",)
    _fields_ = [("flags", ContentFlagsA_bits), ("asUInt8", c_uint8)]
...
class ContentFlagsB(ctypes.Union):
    _anonymous_ = ("flags",)
    _fields_ = [("flags", ContentFlagsB_bits), ("asUInt8", c_uint8)]
...
class ClusterFlagsMHWILDS_bits(ctypes.LittleEndianStructure):
    _fields_ = [
        ...
        (
            ...
            c_uint8,
            1,
        ),  # Don't know what this is doing but it murders FPS if true
...
class ClusterFlagsMHWILDS(ctypes.Union):
    _anonymous_ = ("flags",)
    _fields_ = [("flags", ClusterFlagsMHWILDS_bits), ("asUInt32", c_uint32)]
...
class ClusterFlagsRE9(ctypes.Union):
    _anonymous_ = ("flags",)
    _fields_ = [("flags", ClusterFlagsRE9_bits), ("asUInt32", c_uint32)]
...
class ClusterInfo:
    def __init__(self):
        self.partAABBCenter = (0.0, 0.0, 0.0)  # AABB of full mesh
        self.vertexCount = 0
        self.faceCount = 0
        self.materialID = 0  # Sometimes 1
        self.partIndex = 0
        self.bboxAABBCenterXRaw = 0
        self.bboxExtentX = 0
        self.bboxAABBCenterYRaw = 0
        self.bboxExtentY = 0
    ...
    def read(self, file, version):
        ...
        self.materialID = read_ubyte(file)
        self.partIndex = read_ubyte(file)
        self.bboxAABBCenterXRaw = read_ushort(file)
        self.bboxExtentXRaw = read_ushort(file)
        self.bboxAABBCenterYRaw = read_ushort(file)
        self.bboxExtentYRaw = read_ushort(file)
        self.bboxAABBCenterZRaw = read_ushort(file)
        self.bboxExtentZRaw = read_ushort(file)

        self.bboxAABBCenter = (
            self.bboxAABBCenterXRaw / 65535,
            self.bboxAABBCenterYRaw / 65535,
            self.bboxAABBCenterZRaw / 65535,
        )
        self.bboxExtent = (
            self.bboxExtentXRaw / 65535,
            self.bboxExtentYRaw / 65535,
            self.bboxExtentZRaw / 65535,
        )
        ...
        self.faceBuffer = file.read(
            self.faceCount * 3
        )  # Faces are streamed from streaming mesh, max of 128 faces for non streaming
        ...
        if (self.bitFlag.flags.hasTangentBitsBlock and version < VERSION_RE9) or (
            self.bitFlag.flags.hasTangentBitsBlock
            and self.bitFlag.flags.isMeshletNoTangent
        ):
    ...
    def read(
        ...
        if self.fileHeader.streamingChunkOffset:
            ...
            if self.streamingInfoHeader.entryCount != 0 and streamingBuffer is None:
                raise Exception(
                    "Streaming file associated with mesh is missing, can't import."
                )
        if self.fileHeader.gpuMeshletOffset and self.meshletLayout.gpuDataSize != 0:
            self.clusterInfoLayout = ClusterInfoLayout()
            self.clusterInfoLayout.read(
                file,
                self.meshletLayout.gpuMeshletOffset,
                self.meshletLayout.gpuMeshletHeader.lodClustersOffset,
                version,
            )
